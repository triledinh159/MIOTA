...
public class Detail_device_click_on extends AppCompatActivity {
    ...
    public void onDataUpdateEvent(DataUpdateEvent event) {
        // Nhận dữ liệu từ sự kiện và cập nhật lên biểu đồ
        String newData = event.getData();
        addEntry(newData);
    }
    ...
    private void addEntry(String receivedMessage) {
        try {
                //turn receivedMessage into float
                float numericValue = Float.parseFloat(receivedMessage);
